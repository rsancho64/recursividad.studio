...
def sumatorio(lista):
    """sumatorio iterativo lista enteros"""

    if not esListaEnterosValida(lista):
        raise ValueError(f"lista chunga: {lista}")
...
def sumatorioREC(lista):
    """sumatorio recursivo lista enteros"""

    if not esListaEnterosValida(lista):
        raise ValueError(f"lista chunga: {lista}")

    if not lista: return 0               # trivial 0
    if 1 == len(lista): return lista[0]  # trivial 1
